...
//  isEqualTo
//
//  Purpose: To determine if this Vector3 and another Vector3
//           are equal, according to tolerance
//           VECTOR3_NORM_TOLERANCE.  For an exact comparison,
//           use the equality test operator (operator==).
//  Parameter(s):
//    <1> other: The other Vector3
//  Precondition(s):
//    <1> isFinite()
//    <3> other.isFinite()
//  Returns: Whether this Vector3 is equal to other.
//  Side Effect: N/A
//
	bool isDistanceEqualTo (const Vector3& other) const
	{
		if(fabs(x - other.x) > VECTOR3_NORM_TOLERANCE)
			return false;
		if(fabs(y - other.y) > VECTOR3_NORM_TOLERANCE)
			return false;
		if(fabs(z - other.z) > VECTOR3_NORM_TOLERANCE)
			return false;
		return true;
	}
...
//  project
//
//  Purpose: To project this Vector3 onto another Vector3.
//  Parameter(s):
//    <1> project_onto: The Vector3 to be projected onto 
//  Precondition(s):
//    <1> isFinite()
...
//    <3> !project_onto.isZero()
//  Returns: N/A
//  Side Effect: This Vector3 is set to be its projection onto
//               project_onto.  The projection is the component
//               of this Vector3 with the same (or opposite)
//               direction as project_onto.
//
	void project (const Vector3& project_onto)
	{
		assert(isFinite());
		assert(project_onto.isFinite());
		assert(!project_onto.isZero());
...
//
//  Purpose: To project this Vector3 onto another Vector3
//           without crashing if the other Vector3 is zero.
...
//               zero, this Vector3 is sset to the zero vector.
//
	void projectSafe (const Vector3& project_onto)
	{
		assert(isFinite());
		assert(project_onto.isFinite());
...
//  projectNormal
//
//  Purpose: To project this Vector3 onto another Vector3 that
//           is a normal vector.
//  Parameter(s):
//    <1> project_onto: The Vector3 to be projected onto 
//  Precondition(s):
...
//               direction as project_onto.
//
	void projectNormal (const Vector3& project_onto)
	{
		assert(isFinite());
		assert(project_onto.isFinite());
		assert(project_onto.isNormal());
...
//  reject
//
//  Purpose: To subtract off the projection of this Vector3 onto
//           another Vector3.
//  Parameter(s):
//    <1> project_onto: The Vector3 to be projected onto 
//  Precondition(s):
...
//    <3> !project_onto.isZero()
//  Returns: N/A
//  Side Effect: This Vector3 is set to be its old value minus
//               its projection onto project_onto.  The
//               remainder is the component of this Vector3 at
//               right angles to project_onto.
//
	void reject (const Vector3& project_onto)
	{
		assert(isFinite());
		assert(project_onto.isFinite());
		assert(!project_onto.isZero());
...
//               is zero, this Vector3 is unchanged.
//
	void rejectSafe (const Vector3& project_onto)
	{
		assert(isFinite());
		assert(project_onto.isFinite());
...
//
//  Purpose: To subtract off the projection of this Vector3 onto
//           another Vector3 that is a normal vector.
//  Parameter(s):
//    <1> project_onto: The Vector3 to be projected onto 
//  Precondition(s):
//    <1> isFinite()
...
//               right angles to project_onto.
//
	void rejectNormal (const Vector3& project_onto)
	{
		assert(isFinite());
		assert(project_onto.isFinite());
		assert(project_onto.isNormal());
...
//  reflect
//
//  Purpose: To set this Vector3 to the reflection of its old
...
//    <3> !surface_normal.isZero()
//  Returns: N/A
//  Side Effect: This Vector3 is set to the reflection off a
//               surface with normal vector surface_normal.
//
	void reflect (const Vector3& surface_normal)
	{
		assert(isFinite());
		assert(surface_normal.isFinite());
		assert(!surface_normal.isZero());
...
//  Side Effect: This Vector3 is set to the reflection off a
//               surface with normal vector surface_normal.
//               If surface_normal is zero, this Vector3 is
//               unchanged.
//
	void reflectSafe (const Vector3& surface_normal)
	{
		assert(isFinite());
		assert(surface_normal.isFinite());
...
//               surface with normal vector surface_normal.
//
	void reflectNormal (const Vector3& surface_normal)
	{
		assert(isFinite());
		assert(surface_normal.isFinite());
		assert(surface_normal.isNormal());
